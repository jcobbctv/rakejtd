...
task :load_taljs do
  $taljs = JSON.parse( IO.read( "./taljs.json") );
end

file CONFIG_PATH => [ PRELUDE_PATH, :load_taljs ] do
    generate_script_file_list
    generate_test_file_list
    generate_serve_file_list
    generate_jtd_config_file
end

file PRELUDE_PATH => [ :load_taljs ] do
...
def get_file_list_from_file_block( fileblock )
  files = "";

  if( fileblock )
    if( fileblock[ 'include' ] )
      include_list = FileList.new( fileblock[ 'include' ] )

      if( fileblock[ 'exclude'] )
        exclude_list = FileList.new( fileblock[ 'exclude' ] )
...
def generate_script_file_list
  ...
  $script_files = get_file_list_from_file_block( $taljs[ 'scriptblock'] )
end
...
def generate_test_file_list
  ...
  $test_files = get_file_list_from_file_block( $taljs[ 'testblock' ] )
...
def generate_serve_file_list
  puts "[INFO] Generating Server List"
  $serve_files = get_file_list_from_file_block( $taljs[ 'serveblock' ] )
  #puts $serve_files
end
...
def generate_prelude_file
  ...
  #if the device config is being served from an endpoint then that endpoint should have already
  #defined window.deviceconfig so just start with text_prelude_on_device
  elsif( $params[ :url_to_conf ] != false )
    $text_prelude = $text_prelude_on_device;
  end

  require_block = "this.require = {";

  puts $taljs[ 'require' ]
  require = $taljs[ 'require' ]
  if( require )
    if( require[ "baseUrl"] )
      require_block += "baseUrl:" + "'" + require[ "baseUrl"] + "'"
      if( require[ "paths"] )
        require_block += ","
      end
    end
    ...
    if( require[ "paths"] )
      require_block += "paths: {"
          add_comma = false;
          require[ "paths" ].each_pair do |k,v|
            if( add_comma )
              require_block += ",\n"
            end
            add_comma = true;
            require_block += k.to_s + ":'" + v.to_s + "'"
          end
      require_block += "}"
    end
  end
  ...
  $text_prelude = $text_prelude.gsub( "${REQUIRE}", require_block );
